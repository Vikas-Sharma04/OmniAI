  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={onSubmitHandler} action="" className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        placeholder='The future of artificial intelligence in the world of technology is...' required />
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {articleLength.map((item,index)=>(
            ...
            ${selectedLength.text === item.text ? 'bg-blue-50 text-blue-700' : 'border-gray-300 text-gray-500' }`} key={index}>{item.text}</span>
          ))}
        </div>
        <br />
